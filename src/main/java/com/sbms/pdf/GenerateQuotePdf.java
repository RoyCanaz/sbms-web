...
public class GenerateQuotePdf {
   ...
   // public static final String RESULT = "src/main/webapp/resources/temp/new.pdf";
    public static final String NEWLINE = "\n";
    public static float itemsHeaderFontSize = 11;
    public static float quoteItemsFontSize = 9;
    public static float noteFontSize = 7;
    private static final String IMAGE_DIRECTORY = "/resources/img";
   

    
    public void pdfFile(Qoute q, String currentUser, String path, ServletContext context) throws Exception {
       // String filename =  "src/main/webapp/resources/temp/"+q.getQuoteUuid()+".pdf";   
      // String filename = path+q.getQuoteUuid()+".pdf"; 
         File file = new File(path);
        
         FileOutputStream pdfFileout = new FileOutputStream(file);
         PdfWriter pdfWriter = new PdfWriter(pdfFileout);
         PdfDocument pdf = new PdfDocument(pdfWriter);
          Contact c = q.getContact();
          Company co = q.getCompany();
        ...
        try (Document pdfDocument = new Document(pdf)) {
           ...
           // String imgPath = "src/main/webapp/resources/img/nice_logo.png";
            ...
            Table header = new Table(new UnitValue[]{new UnitValue(UnitValue.PERCENT, 50), new UnitValue(UnitValue.PERCENT, 50)});
             Paragraph he = new Paragraph("Quote").setFontSize(32).setBold().setUnderline();
            ...
            pdfDocument.add(new Paragraph().setBold().setFontColor(ColorConstants.BLUE).setFontSize(15).setTextAlignment(TextAlignment.RIGHT).add(q.getQuoteUuid()));
            ...
            table.addCell(cellInfor("To :", q.getClient().getName(),c.getLastName()+" "+c.getFirstName(), q.getClient().getAddress(), c.getEmail(), c.getMobilePhone(), c.getOfficePhone()));
            table.addCell(fromDetails(q.getQuoteUuid(),
                    DateUtil.currentDate(q.getDateCreated()),                   
                    currentUser,
                    "0773394687", "200038664", "10019550"));
            table.addCell(cellInfor("From :", co.getCompanyName(),
                    currentUser,
                    co.getAddress(), co.getEmail(), co.getOfficePhone(), co.getMobilePhone()));
            ...
            */
             ...
             for(QuoteItem quoteItem : quoteItems){
                ...
                if(p.getCategory().getName().equalsIgnoreCase("Monitor")){
                     des = SpecificationsUtil.setDescription(p.getSpecification());
                }
                 ...
                 Paragraph proCode = new Paragraph(p.getProductCode()==null? "": p.getProductCode()).setFontSize(quoteItemsFontSize);
                 Paragraph desc = new Paragraph(p.getBrand()+", "+p.getModel()+", "+des).setFontSize(quoteItemsFontSize);
                  if(quoteItem.getNote()!=null){
                     desc.add(NEWLINE).add(new Text(quoteItem.getNote()).setFontSize(noteFontSize).setItalic().setBold());
                 }
                  Double unitPriceC = AppUtil.calculateUnitPrice(q.getCurrency().getRate(), p.getSellingPrice());
                  Double linePriceC = unitPriceC*quoteItem.getQuantity();
                  
                  
                 Paragraph quantity = new Paragraph(String.valueOf(quoteItem.getQuantity())).setFontSize(quoteItemsFontSize);
                 Paragraph unitP = new Paragraph(String.valueOf(unitPriceC)).setFontSize(quoteItemsFontSize);
                 Paragraph lineP = new Paragraph(String.valueOf(AppUtil.roundToTwoDecimal(linePriceC))).setFontSize(quoteItemsFontSize);
    ...
    public Cell cellInfor(String header, String name, String contactName, String address, String email, String mobile, String office){
        Paragraph p = new Paragraph()
        .setMultipliedLeading(1.0f)
        .add(new Text(header).setBold().setFontColor(ColorConstants.BLUE)).add(NEWLINE)
        .add(name).add(NEWLINE)
        .add(contactName).add(NEWLINE)
        .add(address).add(NEWLINE)
        .add(email).add(NEWLINE)
        .add(mobile).add(NEWLINE)
        .add(office).add(NEWLINE).add("");
        Cell cell = new Cell()
        .setBorder(Border.NO_BORDER)
        .add(p);
    return cell;
    }
    public Cell fromDetails(String quoteuuid, String dateCreated, String contactName, String phone, String bpn, String vat){
        Paragraph p = new Paragraph().setMultipliedLeading(1.0f)
        .add("Quote No : "+quoteuuid).add(NEWLINE)
